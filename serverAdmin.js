...
/***** Contador de logins de los usuarios ******/
...
app.get('/contador-logins', async (req, res) => {
    try {
        ...
        if (result.recordset.length > 0) {
            const totalLogins = result.recordset[0].total_logins;
...
/***************** Configuraciones de la Red (Mostrando los datos) *****************/

// Obtenemos el nombre de la red WiFi actual
app.get('/get_ssid', (req, res) => {
    const wifiConfigPath = path.join(__dirname, 'WIFI_configuracion');
    const command = `cd "${wifiConfigPath}" && python obtener_SSID.py`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error("Error:", stderr);
            return res.status(500).send("Error al obtener SSID");
        }
        res.send(stdout.trim());
    });
});

// Obtenemos el estado de la conexión WiFi actual
app.get('/get_connection_status', (req, res) => {
    const wifiConfigPath = path.join(__dirname, 'WIFI_configuracion');
    const command = `cd "${wifiConfigPath}" && python estado_conexion.py`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error("Error Estado:", stderr);
            return res.status(500).send("Error al verificar conexión");
        }
        res.send(stdout.trim());
    });
});

// Obtenemos el ip del WiFi
app.get('/get_ip', (req, res) => {
    const wifiConfigPath = path.join(__dirname, 'WIFI_configuracion');
    const command = `cd "${wifiConfigPath}" && python ip_wifi.py`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error("Error:", stderr);
            return res.status(500).send("Error al obtener IP");
        }
        res.send(stdout.trim());
    });
});

// Obtenemos la mac del WiFi
app.get('/get_mac', (req, res) => {
    const wifiConfigPath = path.join(__dirname, 'WIFI_configuracion');
    const command = `cd "${wifiConfigPath}" && python mac_wifi.py`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error("Error:", stderr);
            return res.status(500).send("Error al obtener MAC");
        }
        res.send(stdout.trim());
    });
});
...
app.get('/canal-frecuencia', (req, res) => {
    const scriptPath = path.join(__dirname, 'WIFI_configuracion', 'canal_frecuencia.py');
    exec(`python "${scriptPath}"`, (error, stdout, stderr) => {
        if (error) {
            console.error(`Error al ejecutar canal_frecuencia.py: ${error.message}`);
            return res.json({ resultado: 'Error al obtener canal/frecuencia' });
        }
        if (stderr) {
...
// Obtener velocidad de enlace WiFi
app.get('/velocidad-enlace', (req, res) => {
    const wifiConfigPath = path.join(__dirname, 'WIFI_configuracion');
    const command = `cd "${wifiConfigPath}" && python velocidad_enlace.py`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error("Error:", stderr);
            return res.status(500).send("Error al obtener velocidad");
        }
        res.send(stdout.trim());
    });
});

// Obtener ancho de banda WiFi
app.get('/ancho-banda', (req, res) => {
    const wifiConfigPath = path.join(__dirname, 'WIFI_configuracion');
    const command = `cd "${wifiConfigPath}" && python ancho_banda.py`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error("Error:", stderr);
            return res.status(500).send("Error al obtener ancho de banda");
        }
        res.send(stdout.trim());
    });
});
...
app.get('/protocolo-red', (req, res) => {
    const scriptPath = path.join(__dirname, 'WIFI_configuracion', 'protocolo_red.py');

    exec(`python "${scriptPath}"`, (error, stdout, stderr) => {
        if (error) {
            console.error('Error ejecutando protocolo_red.py:', error);
            return res.json({ protocolo: 'Error al detectar' });
        }

        const resultado = stdout.trim();
        res.json({ protocolo: resultado || 'No detectado' });
    });
});

// Ruta para obtener la contraseña Wi-Fi actual
app.get('/get_wifi_password', (req, res) => {
    const wifiConfigPath = path.join(__dirname, 'WIFI_configuracion');
    const command = `cd "${wifiConfigPath}" && python contraseña_wifi.py`;

    exec(command, (error, stdout, stderr) => {
        if (error) {
            console.error("Error al obtener contraseña Wi-Fi:", stderr);
